...
<head>
  ...
  <title>Update - Colaborador</title>
...
<body class="bg-blue-400">
  <main class="w-full h-svh p-6 flex flex-col justify-center items-center">
    ...
    if (isset($_POST['update'])) {
      ...
      if (empty($nome) || empty($sobrenome) || empty($email) || empty($formacao) || empty($descricao) || empty($id)) {
				if (empty($id)) {
          echo "<h2 class='mt-2 px-3 py-1.5 bg-white text-red-700 text-2xl'>Id field is empty.</h2><br/>";
        }
        ...
        $result = mysqli_query($mysqli, "UPDATE colaboradores SET `nome` = '$nome', `sobrenome` = '$sobrenome', `email` = '$email', `descricao` = '$descricao', `formacao` = '$formacao' WHERE `id` = $id");

        // Display success message
        echo "<p class='px-3 py-1.5 bg-white text-green-500 text-2xl'>Data updated successfully!</p>";
